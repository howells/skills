...
Six reference files are needed by more than one skill. The obvious problem with the arrangement is what a reader sees first: the same file sitting in six places, which looks like someone forgot to factor it out. It's a fair reading, and it's wrong.
...
This decision was taken some time ago and has held since. It's being written down now because it never was, and every reader arriving at those six files has had to work it out again from scratch.

## Decision

`shared/` holds the single source of each of the six files. `scripts/sync-shared.py` materialises a real copy into each consuming skill's own `references/`. Every copy opens with a comment saying not to edit it.
