...
- generation call settings (`maxOutputTokens`, `temperature`, `topP`) passed at the top level of a Mastra `agent.generate`/`stream`/`defaultOptions` call instead of nested under `modelSettings` — silently dropped, so the model runs at its full output budget (common when spreading `@howells/ai` `generationOptions()` or other AI-SDK-flat options into a Mastra agent rather than into `generateText`/`streamText`);
