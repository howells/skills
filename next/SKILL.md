...
One invocation delivers one selected outcome, including documented prerequisites needed to make it work, then stops. A necessary blocker can be implemented in the same task; unrelated work remains separate. The user need not name the tracker, choose an item or separately invoke Plimsoll. An optional issue identifier overrides automatic selection; supplied priority and scope constraints always win.
...
4. Record completion evidence and the appropriate status on each item. Finish only when the original outcome and the prerequisites required for it are verified. Summarize the supporting items alongside the original item at delivery.
