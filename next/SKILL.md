---
name: next
description: "Claim and deliver the next item with Plimsoll; name the thread for it. Not audits (`simplify`) or recovery (`muster`)."
---
...
2. Read unstarted items in the tracker's native priority order. Exclude In Progress, In Review and equivalent started states from automatic selection; they are reserved even if the assignee is empty or shared by multiple sessions. Respect an urgent-only restriction; an empty urgent queue does not authorize lower-priority work. Otherwise choose the highest-priority actionable item. Within the same priority, prefer work that unblocks an active consumer, then the oldest ready item.
3. Fetch the leading item directly from the tracker and check its current status, description, acceptance criteria, dependencies, recent comments and linked implementation. Do not rely on a cached queue, priority label or earlier conversation to establish that it is still open. Skip items already completed, cancelled or superseded; for an explicitly supplied identifier, report that state and stop instead of silently selecting another. Assess unresolved prerequisites using the blocker rule below rather than skipping an item merely because it has a dependency. Skip work that cannot proceed without a human decision, unavailable external access or confirmed active ownership by another session. An unstarted item's historical assignee alone is not evidence of active work. Resume a started item only when this is the owning session or an explicit handoff authorizes takeover; a supplied identifier alone does not override another session's claim. Read only enough related session state to resolve ownership; contact peers only with existing authorization.
4. Give the one-sentence announcement, then immediately claim the selected item and rename the current thread as described below. Do this before delivery investigation, child enumeration, worktree creation or delegation. Do not wait until implementation to make ownership visible.
5. Enumerate the complete child hierarchy, then independently check whether the work is already delivered even when the tracker still says open or urgent. Compare acceptance criteria with linked commits or PRs, the relevant deployed revision when required, and current consumer behaviour. For a bug, capture a small reproduction that can actually expose the reported failure before editing; for a feature, identify which acceptance behaviour is missing. Neither an old failure report nor a merged PR alone establishes the current outcome. If all completion criteria for the selected item and its required descendants already pass, attach the evidence and reconcile the stale status using the project's permitted completion transition; that completes this invocation without inventing a code change. If only part is delivered, state exactly what remains and implement only that gap. If verification is unavailable, keep the item open and report the missing evidence rather than assuming completion. Re-fetch status before changing it so a concurrent update is not overwritten.
6. Proceed without a selection approval round. If no eligible work exists, report the actual blockers and stop; do not turn selection into a backlog audit.

## Claim the item and name the thread

Re-fetch the selected item's status and ownership immediately before claiming it. Resolve the owning team's actual **In Progress** state (or native equivalent), update the item to that state and record the current session's identifier or link using the tracker's supported ownership field or a brief claim note. An assignee alone cannot distinguish concurrent sessions using the same account. Preserve unrelated fields. Read the item back and confirm the started state and this session's ownership before proceeding. If the write fails or the claim is ambiguous, do not start implementation or delegate that item.

Use an atomic conditional transition or an existing shared claim mechanism when the integration supports one. Otherwise, re-fetch immediately before writing and verify ownership immediately afterwards; these checks reduce races but are not an atomic lock. If another session has claimed the item, leave its state and ownership intact and choose the next eligible item; for an explicitly supplied identifier, report the conflict and stop. Do not overwrite a competing claim or treat a successful status write alone as exclusive ownership.

Once the claim is confirmed, rename the **current** thread or session to `<ITEM-ID>: <short plain-language outcome>`, for example `APP-123: Show matching search results`. Use the active harness's supported rename tool, command or UI control and verify the resulting title. Keep the original selected item in the title when delivering children or prerequisites. If renaming is unavailable, state that limitation once and continue; a heading in a reply is not a thread rename. Do not edit private harness databases or invent unsupported commands.

Apply the same claim procedure to unstarted children as they are discovered and to necessary prerequisites before investigating or implementing them; a parent's status does not reserve its children. The coordinator owns these claims when delegating within this invocation. Preserve another session's active ownership and the required child scope. Do not move completed or cancelled items to In Progress merely to inspect their evidence.

Keep claims visible through delivery and follow the project's review, blocked and completion transitions. If this invocation relinquishes an item without handing it off, re-fetch ownership, record what remains and release only its own claim into the appropriate unstarted or blocked state. Never reset another session's status or leave abandoned work falsely marked as active.

## Include the complete child hierarchy

Selecting a parent includes all its child items and nested descendants in this invocation's delivery scope, even if they have a lower priority or are not individually necessary for the parent's headline acceptance. Priority restrictions govern selection of the initial item, not which of its children get delivered. An explicit user scope restriction still wins. Selecting a child does not pull in its siblings or ancestors.

Before implementation or declaring the selected item already delivered, fetch every page of its child relationships recursively, including completed and cancelled items. Read each descendant's current status, spec, acceptance criteria, dependencies and relevant delivery evidence. Deduplicate items by tracker identity. A partial child list or failed lookup cannot establish that the hierarchy is complete; continue independently known work where safe and report the coverage gap.

Apply the same live-status and acceptance checks to each child. Verify completed children's evidence rather than reimplementing them; reconcile open-but-delivered items under the project's rules. Do not revive cancelled, duplicate or superseded work, but follow any replacement still required for the selected outcome and explain exclusions. If a completed child lacks required evidence, establish that evidence or report it as unresolved; do not count its status alone as verified delivery.

Deliver every remaining child and its necessary prerequisites, blockers first, within the existing task and delivery path where project policy allows. Use parallel agents for independent children with distinct ownership. Keep each child's spec, evidence and status on its own tracker item, and link the parent and included children in the PR. A child is included by its place in the hierarchy; it does not need to pass the separate necessary-blocker test. Unrelated findings stay separate and must not be attached as children merely to expand this task.

A child waiting on another active owner, unavailable access, a human decision or an observation period remains part of the scope. Complete independent work, preserve ownership and report exactly what prevents finishing it. Do not drop that child, silently defer it to another invocation or mark the parent complete. Near handoff, refresh the hierarchy once to catch newly added or changed children and account for them before claiming completion; this is not a polling loop. The selected outcome is delivered only when the parent's acceptance and every non-excluded descendant's acceptance have the required evidence. Summarize child completion and any outstanding children briefly in the final handoff.
...
Attach acceptance evidence to the selected item, each included child and each prerequisite handled. Respect the project's tracker rules: ready for review, merged, deployed and verified are separate states. Evidence-only work remains open until its required evidence exists; another code change cannot substitute for an observation period.

## Tidy this task after merge

After the forge confirms the PR merged and required delivery work is complete, clean up this task's branch and worktree as part of finishing. An open PR or one closed without merging is not cleanup authorization. Follow the project's and machine's worktree policy; this cleanup covers only the work owned by this task.

1. Verify the final PR head was merged and the branch has no later, undelivered commits. For squash or rebase merges, verify the forge's merged head and resulting change rather than relying on an ancestry check alone. Record the PR, resulting commit and delivery evidence before removing local state.
2. Check the worktree for tracked changes, untracked files, retained ignored artifacts and related stashes. Preserve anything unique or ambiguous; a clean tracked diff alone is insufficient. Confirm that no agent, dev server, build or other live process still needs the directory. Stop only this task's owned processes when their work is finished.
3. Perform removal from a safe checkout outside the target worktree. If the active harness still owns that directory, use its supported handoff first; otherwise retain the worktree and state the exact remaining cleanup instead of deleting the active session's directory.
