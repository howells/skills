...
A ship carries a load line painted on its hull. Below it the vessel moves. Above it the vessel is overloaded and sits too low to make way. This skill finds the line for a piece of work and throws the excess overboard.
...
**Process weight** - everything carried to make the payload safe to move. Checks, gates, polls, reviews, plans, PR hygiene, subagent ceremony. Necessary. Not free. Weight is only worth carrying in proportion to what it protects.

**The rule that decides every call: process weight is justified by the payload it protects, never by its own completeness.** A check nobody would act on is weight. A gate that cannot fail given what you changed is weight. A poll that changes nothing is weight. Running one more because the last one passed is the definition of overloading.

This skill never licenses shipping something broken. It moves verification off the workspace and onto the payload. A demo that crashes in front of the audience is the more expensive failure, and it is the one that has actually happened.
...
1. **Escalating scope after a pass.** Focused test, then package typecheck, then workspace typecheck, then workspace lint, then the full suite. Each rung was triggered by the previous rung going green, not by a risk anyone named.
...
5. **A green gate reported as the outcome.** "136 passed, 1 failed" is not a deliverable. Nobody outside the session can see it.
...
8. **Adopting the obstacle as the task.** The build was slow, so now you are optimising the build. Note it as a tracker item and carry on with what you were sent to do.
9. **Stopping other work so a check can run cleanly.** Halting parallel agents to get a still tree is a large cost for a signal that will be stale a minute later.

**Ceremony** - structure produced instead of payload
...
11. **Per-task ritual.** A test-writer, a spec-reviewer and a code-quality reviewer dispatched for a change one person could read in a minute.
12. **Splitting delivery for tidiness.** Separate branches and PRs per item when one would land the lot.
...
15. **Trial and error at length.** Three failed attempts at the same thing is not a fourth attempt's problem. Stop, and get an outside read - `fable-review` for a judgement call, `glm-review` for a bounded conformance check.
...
6. **Verify against the payload, not the workspace.** Open the page. Run the query. Play the video. Click the deploy. `fieldtest` if it is an app in a browser. This step is not optional and it is the one that keeps getting skipped in favour of the suite.
...
Deleting data, rotating credentials and touching production are outside this calibration. Those are always slow.

## What this is not

- Not permission to ship broken work, or to claim something works without opening it. The most expensive failures in the record are unverified claims, not slow gates.
- Not an argument against tests. It is an argument against tests aimed at the workspace when the payload is unopened.
