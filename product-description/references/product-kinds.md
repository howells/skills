...
- **Verification:** "device" means browser and viewport (desktop, mobile web), account role, network condition (offline toggle, throttled), and a second account in a second browser. Use the verification environment and permitted actions recorded in Phase 0. Staging or an isolated test setup is preferable for mutating flows; a throwaway production account alone does not authorize emails, payments, invites, publishing or shared-data changes. Browser devtools stand in for the console: read the network tab to see what a submit sent, the application tab to see what was stored.
