...
Use independent read-only agents for distinct areas when the scope benefits and the harness supports them. Match the host's model tiers to the work: the frontier tier for design across packages, the workhorse for tracing behavior, the cheap tier for bounded inspection. Give each agent its scope, repository decisions, stage, relevant rubric, existing evidence and expected findings. The coordinator checks shared boundaries and consolidates findings. Continue sequentially if delegation is unavailable.
